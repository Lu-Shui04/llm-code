...
questions = [
    "2025-06-16，股市利好。股票智传教育A股今日开盘价66人民币，一度飙升至70人民币，随后回落至65人民币，最终以68人民币收盘，成交量达到123000。",
    "2025-06-06，股市利好。股票黑马程序员A股今日开盘价200人民币，一度飙升至211人民币，随后回落至201人民币，最终以206人民币收盘。"
]

messages=[
    {"role":"system","content":f"你是一个股票信息抽取专家，按照以下{schema}，从文本中抽取相关信息。"}
    ]

for example in examples_data:
    messages.append(
        {"role":"user","content":json.dumps(example['content'],ensure_ascii=False)}
    )
    messages.append(
...
for q in questions:
    response=client.chat.completions.create(
        model="qwen3-max",
        messages=messages+[{"role":"user","content":f"根据上述例子，把这段{q}文本中的信息抽取出来"}],
        stream=False
    )
